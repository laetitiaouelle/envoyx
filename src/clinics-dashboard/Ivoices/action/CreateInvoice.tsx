        ...
        <form onSubmit={handleSubmit((data) => alert(data))} className='grid grid-cols-10 gap-5'>
            <div className='col-span-10' >
                <motion.label whileTap={{scale:0.99}} htmlFor='file' className=' px-3 cursor-pointer flex items-center justify-start gap-3 text-text-dark w-full h-10 border border-main-blue  rounded-[10px] border-opacity-20 ' >
                    <span className='block text-main-red' ><LiaFileInvoiceSolid size={20} /></span>
                    <span className='block text-sm' >{"Upload invoice file"}</span>
                </motion.label>
            ...
            <div className='col-span-5' >
                <div className=' w-full' >
                <span className='block text-sm text-text-light mb-2' > {"Insurance"} </span>
                    <input {...register('insuranceName', { required: true })} 
                    className='outline-none focus:right-0 ring-0 bg-[#fff] h-10 border w-full border-main-purple border-opacity-20 rounded-[10px] px-3 placeholder:text-xs placeholder:text-text-dark text-text-dark ' 
            ...
            <div className='col-span-7' >
                <div className=' w-full' >
                <span className='block text-sm text-text-light mb-2' > {"Patient full name"} </span>
            ...
            <div className='col-span-3' >
                <div className=' w-full' >
                <span className='block text-sm text-text-light mb-2' > {"Register date"} </span>
            ...
            <div className='col-span-5' >
                <div className=' w-full' >
                <span className='block text-sm text-text-light mb-2' > {"Invoice amount"} </span>
            ...
            <div className='col-span-5' >
                <div className=' w-full' >
                <span className='flex items-center gap-1 text-sm text-text-light mb-2' > {"Status"} <span className='block text-xs italic' > {"(Optional)"} </span> </span>
